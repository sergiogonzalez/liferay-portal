...
for (int displayActivityCounterNameIndex : displayActivityCounterNameIndexes) {
	...
	String title = LanguageUtil.get(pageContext, "group-statistics-container") + StringPool.SPACE;
	...
	if (chartType.equals("tag-cloud")) {
		...
		title = title + LanguageUtil.format(pageContext, "tag-cloud-based-on-x", new Object[] {LanguageUtil.get(pageContext, "social.counter.group.statistics." + displayActivityCounterName)});
		...
		title = title + LanguageUtil.get(pageContext, "social.counter.group.statistics." + displayActivityCounterName);
	}
