...
<div class="aui-field-row">
	...
	<aui:select inlineField="<%= true %>" label="" name='<%= "preferences--displayActivityCounterName" + index + "--" %>'>
		...
		%>
			...
			<aui:option label='<%= LanguageUtil.get(pageContext, "social.counter.group.statistics."+ activityCounterName) %>' selected="<%= activityCounterName.equals(displayActivityCounterName) %>" value="<%= activityCounterName %>" />
