...
public class WikiTestUtil {
	...
	public static WikiPage[] addChildPageVersionsWithDifferentParentTitle(
		...
		WikiPage initialParentPage = WikiTestUtil.addPage(
			...
			"TestInitialParentPage", true);

		WikiPage childPage = WikiTestUtil.addPage(
			TestPropsValues.getUserId(), nodeId, "TestChildPage",
			ServiceTestUtil.randomString(), "TestInitialParentPage", true,
			serviceContext);

		WikiPage finalParentPage = WikiTestUtil.addPage(
			TestPropsValues.getUserId(), groupId, nodeId, "TestFinalParentPage",
			true);

		WikiPageLocalServiceUtil.changeParent(
			TestPropsValues.getUserId(), nodeId, "TestChildPage",
			"TestFinalParentPage", serviceContext);

		childPage = WikiPageLocalServiceUtil.getPageByPageId(
			childPage.getPageId());

		return new WikiPage[] {childPage, initialParentPage, finalParentPage};
	}
