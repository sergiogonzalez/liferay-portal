...
public class DLFolderLocalServiceImpl extends DLFolderLocalServiceBaseImpl {
	...
	public DLFolder updateFolder(
		...
		try {
			...
			long[] originalDLFileEntryTypeIds = getOriginalDLFileEntryTypeIds(
				folderId, dlFolder);
			...
			if (restrictionType ==
				...
				if (ArrayUtil.isEmpty(originalDLFileEntryTypeIds)) {
					originalDLFileEntryTypeIds = ArrayUtil.append(
	...
	/**
	 * @deprecated As of 7.0.0, replaced {@link #updateFolder(long, String,
	...
	protected long[] getOriginalDLFileEntryTypeIds(
			long folderId, DLFolder dlFolder)
		throws PortalException {

		long[] originalFileEntryTypeIds = new long[0];

		long[] groupIds = PortalUtil.getCurrentAndAncestorSiteGroupIds(
			dlFolder.getGroupId());

		List<DLFileEntryType> originalDLFileEntryTypes =
			dlFileEntryTypeLocalService.getFolderFileEntryTypes(
				groupIds, folderId, true);

		for (DLFileEntryType dlFileEntryType : originalDLFileEntryTypes) {
			originalFileEntryTypeIds = ArrayUtil.append(
				originalFileEntryTypeIds, dlFileEntryType.getFileEntryTypeId());
		}

		return originalFileEntryTypeIds;
	}
