...
public class JournalFolderFinderImpl extends BasePersistenceImpl<JournalFolder>
	...
	public static final String COUNT_JA_BY_G_F =
		JournalFolderFinder.class.getName() + ".countFE_ByG_F";

	public static final String FIND_F_BY_G_F =
		JournalFolderFinder.class.getName() + ".findFByG_F";

	public static final String FIND_JA_BY_G_F =
		JournalFolderFinder.class.getName() + ".findJAByG_F";
		...
		try {
			...
			while (itr.hasNext()) {
				...
				if (modelFolder == 1) {
					...
					//obj = JournalArticleUtil.findByG_F_N(
	...
	protected String updateSQL(String sql, long folderId) {
		sql = StringUtil.replace(sql, "[$FOLDER_PARENT_FOLDER_ID$]",
			getFolderId(folderId, "JournalFolder"));

		sql = StringUtil.replace(sql, "[$JOURNAL_ARTICLE_FOLDER_ID$]",
			getFolderId(folderId, "JournalArticle"));
