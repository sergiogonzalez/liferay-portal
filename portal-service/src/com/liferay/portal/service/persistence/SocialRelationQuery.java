...
public class SocialRelationQuery {
	...
	public String getJoinClause() {
		...
		if (isAnyRelationship()) {
			clause = PortalCustomSQLUtil.get(JOIN_BY_SOCIAL_RELATION_TYPE);
		}
		else if (isFriendlyRelationship()) {
			clause = PortalCustomSQLUtil.get(JOIN_BY_SOCIAL_RELATION);
		}
		else if (isMutualRelationship()) {
			clause = PortalCustomSQLUtil.get(JOIN_BY_SOCIAL_MUTUAL_RELATION);
		}
		else {
			throw new IllegalStateException(
				QUERY_TYPE_ERROR_MESSAGE + _queryType);
		}
	...
	private String addInOrEqual(String sql, String fieldName, Long[] values) {
		String result = null;
	...
	private String addMutualRelationWhereClause() {
		String sql = PortalCustomSQLUtil.get(JOIN_BY_SOCIAL_MUTUAL_RELATION);
	...
	private String addPersonalRelationWhereClause() {
		String sql = PortalCustomSQLUtil.get(JOIN_BY_SOCIAL_RELATION_TYPE);

		return addInOrEqual(
			sql, "SocialRelation.type_", _firstUserRelationTypeIds);
	}

	private boolean isAnyRelationship() {
		return _queryType == QueryType.ANY_RELATIONSHIP;
	}

	private boolean isFriendlyRelationship() {
		return _queryType == QueryType.NO_ENEMIES;
	}

	private boolean isMutualRelationship() {
		return _queryType == QueryType.MUTUAL;
	}

	private static final String JOIN_BY_SOCIAL_RELATION_TYPE =
		UserFinder.class.getName() + ".joinBySocialRelationType";

	private static final String QUERY_TYPE_ERROR_MESSAGE =
	...
	private static final String JOIN_BY_SOCIAL_MUTUAL_RELATION =
		UserFinder.class.getName() + ".joinBySocialMutualRelation";

	private static final String JOIN_BY_SOCIAL_RELATION =
