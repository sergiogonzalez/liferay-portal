...
public class ReadingTimeCalculatorImpl implements ReadingTimeCalculator {
	...
	@Override
	public Optional<Duration> calculate(GroupedModel model) {
		ReadingTimeModelInfo readingTimeModelInfo =
			_serviceTrackerMap.getService(model.getModelClassName());
		...
		return calculate(
			readingTimeModelInfo.getContent(model),
			readingTimeModelInfo.getContentType(model),
			readingTimeModelInfo.getLocale(model));
	}
