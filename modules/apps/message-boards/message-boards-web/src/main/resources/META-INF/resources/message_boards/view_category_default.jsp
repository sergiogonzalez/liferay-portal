...
<liferay-ui:panel-container cssClass="message-boards-panels" extended="<%= false %>" id="messageBoardsPanelContainer" persistState="<%= true %>">
	...
	<c:if test="<%= categoriesCount > 0 %>">
		<liferay-ui:panel collapsible="<%= categoriesPanelCollapsible %>" extended="<%= categoriesPanelExtended %>" id="messageBoardsCategoriesPanel" markupView="lexicon" persistState="<%= true %>" title='<%= LanguageUtil.get(request, (category != null) ? "subcategories[message-board]" : "categories[message-board]") %>'>
	...
	<liferay-ui:panel collapsible="<%= threadsPanelCollapsible %>" cssClass="threads-panel" extended="<%= threadsPanelExtended %>" id="messageBoardsThreadsPanel" markupView="lexicon" persistState="<%= true %>" title='<%= LanguageUtil.get(request, "threads") %>'>
		<liferay-ui:search-container
			curParam="cur2"
