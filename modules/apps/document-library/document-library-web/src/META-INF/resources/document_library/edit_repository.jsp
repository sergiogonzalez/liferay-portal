			...
			<c:choose>
				...
				<c:otherwise>
					...
					<div class="repository-settings-display">
						...
						for (RepositoryParameter repositoryParameter : repositoryConfiguration.getRepositoryParameters()) {
							String parameterValue = typeSettingsProperties.getProperty(repositoryParameter.getName());

							if (Validator.isNotNull(parameterValue)) {
						%>

								<dt>
									<%= repositoryParameter.getLabel(locale) %>
								</dt>
								<dd>
			...
			%>
				...
				<div class="settings-parameters" id="<portlet:namespace />repository-<%= unqualifiedClassName %>-configuration">
					...
					%>
						...
						<aui:input label="<%= repositoryParameter.getLabel(locale) %>" name='<%= "settings--" + HtmlUtil.escapeAttribute(repositoryParameter.getName()) + "--" %>' type="text" value="" />
